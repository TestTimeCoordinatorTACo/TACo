...
class DetectionTSCheckpointer(DetectionCheckpointer):
    def _load_model(self, checkpoint):
        ...
        for key in checkpoint_copy['model'].keys():
            #print(key)
            checkpoint['model']['modelTeacher.'+key]=checkpoint_copy['model'][key]
            checkpoint['model']['modelStudent.'+key]=checkpoint_copy['model'][key]
            if 'roi_heads' in key:
                checkpoint['model']['modelTeacher.calibration_'+key]=checkpoint_copy['model'][key]
                checkpoint['model']['modelStudent.calibration_'+key]=checkpoint_copy['model'][key]
        #exit()
...
class DDDetectionTSCheckpointer(DetectionCheckpointer):
    def _load_model(self, checkpoint):
        if checkpoint.get("__author__", None) == "Caffe2":
            # pretrained model weight: only update student model
            if checkpoint.get("matching_heuristics", False):
