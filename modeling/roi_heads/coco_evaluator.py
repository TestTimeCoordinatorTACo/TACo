...
def get_coco_metrics(
    ...
    # pairwise ious
    _ious = {k: _compute_ious(**v) for k, v in _bbs.items()}
    #print(_ious)# {(img_id, class_id):np.zeros((len(dt), len(gt))), ():}

    # accumulate evaluations on a per-class basis
    _evals = defaultdict(lambda: {"scores": [], "matched": [], "NP": [], "cali_m": []})
    ...
    for img_id, class_id in _bbs:
        ...
        acc = _evals[class_id]
        #score_list.extend(score_list_per_im)
        
        acc["scores"].extend(ev["scores"])
        acc["matched"].extend(ev["matched"])
        acc["NP"].append(ev["NP"])
        acc["cali_m"].extend(ev["cali_m"])
    ...
    for class_id in _evals:
        ...
        acc["NP"] = np.sum(acc["NP"])
        acc["cali_m"] = np.array(acc["cali_m"]).astype(np.bool)
    ...
    for class_id in _evals:
        ...
        res[class_id] = {
            "class": class_id,
            "conf_scr": ev["scores"],
            **_compute_ap_recall(ev["scores"].detach().cpu().numpy(), ev["matched"], ev["NP"], ev["cali_m"])
        }
    return res
...
def get_ece_input(
    ...
    # accumulate evaluations on a per-class basis
    _evals = defaultdict(lambda: {"scores": [], "matched": [], "x_middle": []})
    ...
    for img_id, class_id in _bbs:
        ...
        acc["scores"].extend(ev["cpu_scores"])
        acc["matched"].extend(ev["matched"])
        acc["x_middle"].extend(ev["x_middle"])
    ...
    for class_id in _evals:
        acc = _evals[class_id]
        confidences.extend(acc["scores"])
        matched.extend(acc["matched"])
        x_middle.extend(acc["x_middle"])
        #acc["scores"] = np.array(acc["scores"].cpu().detach().numpy())
        #acc["matched"] = np.array(acc["matched"]).astype(np.bool)
        #acc["x_middle"] = np.array(acc["x_middle"])
    
    #print(_evals)
    #exit()
    confidences=np.array(confidences)
    matched=np.array(matched).astype(np.bool)
    x_middle=np.array(x_middle)
       
    return confidences, matched, x_middle
...
def _evaluate_image(dt, gt, ious, iou_threshold, max_dets=None, area_range=None):
    ...
    for d_idx, d in enumerate(dt):
        ...
        iou = min(iou_threshold, 1 - 1e-10) # to filter high iou, increase iou_threshold
        m = -1
        dt_nm[d_idx] = 1
        for g_idx, g in enumerate(gt):
            ...
            if g_idx in gtm:
                continue
            # for cali: filter more real FP
            if ious[d_idx, g_idx] < 0.5:
                dt_nm[d_idx] = -1 
                #print("==0 iou:",ious[d_idx, g_idx])
            else:
                dt_nm[d_idx] = 1
            ...
            #print(">0.9 iou:",iou)
            m = g_idx
        #print("d_idx:", d_idx, "m:", m, "iou:", iou, "cali_m:", dt_nm[d_idx])
    ...
    cali_matched = [dt_nm[d_idx]==-1 for d_idx in range(len(dt)) if not dt_ignore[d_idx]] #in matched's -1, the one with low iou is marked to count FP, otherwise, ignore
    #print(scores)
    #print(matched)
    #print(cali_matched)
    #print("original #FP:", len([i for i in matched if i==False]), "filtered #FP:", len([i for i in cali_matched if i==True]))
    #print("score for filtered FP:", [dt[d_idx].get_confidence() for d_idx in range(len(dt)) if dt_nm[d_idx]==-1 and not dt_ignore[d_idx]])
    score_list_per_im = [dt[d_idx].get_confidence().cpu().detach().numpy() for d_idx in dtm]  
    n_gts = len([g_idx for g_idx in range(len(gt)) if not gt_ignore[g_idx]])
    x_middle = [(dt[d_idx].get_absolute_bounding_box(format=BBFormat.XYX2Y2)[0]+dt[d_idx].get_absolute_bounding_box(format=BBFormat.XYX2Y2)[2])/2. for d_idx in range(len(dt)) if not dt_ignore[d_idx]]
    cpu_scores =  [dt[d_idx].get_confidence().cpu().detach().numpy() for d_idx in range(len(dt)) if not dt_ignore[d_idx]]
    return {"scores": scores, "matched": matched, "NP": n_gts, "cali_m": cali_matched, "x_middle": x_middle, "cpu_scores": cpu_scores}


def _compute_ap_recall(scores, matched, NP, cali_m, recall_thresholds=None):
