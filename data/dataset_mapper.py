...
class DatasetMapperTwoCropSeparate(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        dataset_dict_origin = copy.deepcopy(dataset_dict)
        dataset_dict_origin["image"] = torch.as_tensor(
        ...
        if "annotations" in dataset_dict:
            ...
            for obj in dataset_dict.pop("annotations"):
                if obj.get("iscrowd", 0) == 0:
                    flip_flag, oannotation = utils.transform_instance_annotations(
                                        obj,
                                        transforms,
                                        image_shape,
                                        keypoint_hflip_indices=self.keypoint_hflip_indices,
                                        )
                    annos.append(oannotation)
                    flip_flag_list.append(flip_flag)       
            #print(dataset_dict["file_name"], len(flip_flag_list))        
            '''
            annos = [
        ...
        #print("image shape:",image.shape,"image_weak shape:",image_weak_aug.shape,"image_strong shape:",image_strong_aug.shape)
        #exit()
        ...
        #assert dataset_dict["image"].size(1) == dataset_dict_origin["image"].size(1)
        #assert dataset_dict["image"].size(2) == dataset_dict_origin["image"].size(2)
        return (dataset_dict, dataset_dict_key, dataset_dict_origin, flip_flag_list)
