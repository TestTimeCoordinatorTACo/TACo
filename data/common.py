...
class AspectRatioGroupedSemiSupDatasetTwoCrop(AspectRatioGroupedDataset):
    ...
    def __init__(self, dataset, batch_size):
        ...
        self.unlabel_dataset = dataset
        self.batch_size_unlabel = batch_size

        self._unlabel_buckets = [[] for _ in range(2)]
        self._unlabel_buckets_key = [[] for _ in range(2)]
        self._unlabel_buckets_origin = [[] for _ in range(2)]
        self._unlabel_buckets_flip_flag = [[] for _ in range(2)]
        # Hard-coded two aspect ratio groups: w > h and w < h.
        # Can add support for more aspect ratio groups, but doesn't seem useful

    def __iter__(self):
        unlabel_bucket = []
        for d_unlabel in self.unlabel_dataset:
            # d is a tuple with len = 2
            # It's two images (same size) from the same image instance
            ...
            if len(unlabel_bucket) != self.batch_size_unlabel:
                ...
                unlabel_buckets_origin = self._unlabel_buckets_origin[unlabel_bucket_id]
                unlabel_buckets_origin.append(d_unlabel[2])
                unlabel_buckets_flip_flag = self._unlabel_buckets_flip_flag[unlabel_bucket_id]
                unlabel_buckets_flip_flag.append(d_unlabel[3])


            # yield the batch of data until all buckets are full
            if (
                len(unlabel_bucket) == self.batch_size_unlabel
            ):
                # label_strong, label_weak, unlabed_strong, unlabled_weak
                yield (
                    unlabel_bucket[:],
                    unlabel_buckets_key[:],
                    unlabel_buckets_origin[:],
                    unlabel_buckets_flip_flag[:],
                )
                del unlabel_bucket[:]
                del unlabel_buckets_key[:]
                del unlabel_buckets_origin[:]
                del unlabel_buckets_flip_flag[:]
