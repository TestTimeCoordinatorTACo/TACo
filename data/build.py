...
# uesed by unbiased teacher trainer!!!!!!!change it to pure unsupervised
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    ...
    #print("#############################3",unlabel_dataset.__getitem__(0))

    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN
    logger = logging.getLogger(__name__)
    logger.info("Using training sampler {}".format(sampler_name))
    if sampler_name == "TrainingSampler":
        unlabel_sampler = TrainingSampler(len(unlabel_dataset), False)################shuffle the input
    elif sampler_name == "RepeatFactorTrainingSampler":
